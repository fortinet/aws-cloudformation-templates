...
class GetHealthCheckStatus(object):
    def __init__(self, ip, port):
        ...
        try:
            ...
            s.close()
            self.status = False
            logger.error('<--!! Exception in GetHealthCheckStatus: {}'.format(ERROR))
            logger.info('<-- Host+Port {}:{} is UP = {}'.format(self.ip, self.port, self.status))
...
class GetRoutes(object):
    def __init__(self, routetables, eni1, eni2):
        ...
        self.routemap = {}
        for rt in self.rts:
            logger.debug('--> Getting routes in rt {} which target {} or {}'.
                         format(rt, self.eni1, self.eni2))
            try:
                response = ec2client.describe_route_tables(RouteTableIds=[rt])
                logger.debug('<-- Response: {}'.format(response))
            except Exception as ERROR:
                logger.error('<--!! Exception in GetRoutes: {}'.format(ERROR))
                sys.exit()
            for route in response['RouteTables'][0]['Routes']:
                if 'NetworkInterfaceId' in route:
                    if (self.eni1 in route['NetworkInterfaceId'] or
                        self.eni2 in route['NetworkInterfaceId']):
                        self.count += 1
                        self.routemap[self.count] = [rt, route['DestinationCidrBlock'],
...
def lambda_handler(event, context):
    ...
    az1hc = GetHealthCheckStatus(az1ip, hcport)
    az1routes = GetRoutes(az1rts, az1eni, az2eni)
